...
- Added schemes to make the foreground prefer small cores, but allow scaling to big cores IF necessary. (This saves a small amount of power in light usage situations, such as scrolling, etc.). The focus of top-app is exclusively on the big cores, avoiding early allocation and overload on these cores that are naturally focused on the user.
